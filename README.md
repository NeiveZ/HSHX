...
git clone https://github.com/NeiveZ/HSHX.git
cd HSHX
pip install -r requirements-dev.txt --break-system-packages   # includes optional bcrypt
pytest                       # 35 tests, including the official RFC 1320 MD4 test vectors
ruff check .                 # lint

# Scriptable / CI mode — no interactive shell needed
python3 hshx.py -m hash/crack -s HASH=5f4dcc3b5aa765d61d8327deb882cf99 -s ALGO=md5 --no-color
```
...
with [APIX](https://github.com/NeiveZ/APIX) so both tools stay consistent. NTLM hashing
falls back to a pure-Python MD4 implementation (`utils/md4.py`) when the system OpenSSL
doesn't expose MD4 (common on OpenSSL 3.x / Kali and Debian 12+), so `hash/crack ALGO=ntlm`
